...
class Model(Model):
    ...
    def build_model(self):
        # dtype should be the same, tf_float_dtype = tf.float64 leads a CNN problem.
        # tf.layers.Conv2D does not work with tf.float64
        ...
        self.embedding_size_context = (self.FLAGS.target_window_size * 2) + 1
        self.kernel_size = (self.FLAGS.target_window_size * 2) + 1
...
# =============================================================================
    ...
    def get_emission(self):
        ...
        target_embedded_word = tf.reshape(target_embedded_word, (targets_shape[0], 
                                                      targets_shape[1],
                                                      self.embedding_size_context, self.FLAGS.embedding_size))
        
        target_embedded_null = tf.reshape(target_embedded_null, (targets_shape[0], 
                                                      targets_shape[1],
                                                      self.embedding_size_context, self.FLAGS.embedding_size))
        
        target_embedded_word = tf.cast(target_embedded_word, dtype=self.tf_float_dtype)
        target_embedded_null = tf.cast(target_embedded_null, dtype=self.tf_float_dtype)
        
        input_scan = (target_embedded_word, target_embedded_null)
        
        def get_cnn(value):
            target_embedded_word = value[0]
            target_embedded_null = value[1]
            
            target_embedded_word_shape = tf.shape(target_embedded_word)
            target_embedded_word = tf.reshape(target_embedded_word,
                                         (target_embedded_word_shape[0],
                                          target_embedded_word_shape[1],
                                          target_embedded_word_shape[2],1))
            
            target_embedded_null = tf.reshape(target_embedded_null,
                                         (target_embedded_word_shape[0],
                                          target_embedded_word_shape[1],
                                          target_embedded_word_shape[2],1))
            
            target_embedded = tf.concat([target_embedded_word, target_embedded_null], axis=0)
            
            cnnLayer = tf.layers.Conv2D(filters=1,
            ...
            target_embedded = cnnLayer(target_embedded)
            target_embedded = tf.squeeze(target_embedded)
            
            return target_embedded, 0
            
        
        target_embedded, _ = tf.map_fn(get_cnn,input_scan, dtype=(self.tf_float_dtype,
                                                                  self.tf_float_dtype) )
        
        
        target_embedded = tf.reshape(target_embedded, (targets_shape[0],
                                                     targets_shape[1]*2,
                                                     self.FLAGS.embedding_size-(self.FLAGS.target_window_size*2)))
        ...
        target_state = nnLayer(nnLayerBridge(target_embedded))
        
        
        #----------------------------------------------------------------------

        target_state = tf.nn.dropout(x=target_state,
                                       keep_prob=tf.cast(self.keep_prob, dtype=self.tf_float_dtype),
        ...
        def get_source(value):
            ...
            emission_prob = tf.gather(target_state, sources, axis=-1)
