...
# =============================================================================
    ...
    def build_variational_encoder(self):
        with tf.variable_scope('target_y_encoder'):
            self.y_hidden_variable, \
            self.y_hidden, \
            self.y_hidden_variable_null, \
            self.y_hidden_null, \
            self.y_KL_divergence = self.get_latent_variable(self.targets, 
                                                          self.targets_null, 
                                                          self.target_lengths,
                                                          self.FLAGS.target_vocabulary_size)
            
        with tf.variable_scope('source_x_encoder'):
            self.x_hidden_variable, \
            self.x_hidden, \
            self.x_hidden_variable_null, \
            self.x_hidden_null, \
            self.x_KL_divergence = self.get_latent_variable(self.sources, 
                                                          self.sources_null, 
                                                          self.source_lengths,
...
# =============================================================================
    ...
    def build_reconstruction(self):
        with tf.variable_scope('reconstruction', reuse=tf.AUTO_REUSE):
            self.e_y_reconstruction_expectation,\
            self.e_y_reconstruction_expectation_log  = self.get_reconstruction(self.e_y_hidden_prob, 
                                                                                 self.targets, 
                                                                                 self.target_lengths)
                
            self.e_y_output_predicted = tf.argmax(self.e_y_prob,axis=-1)
            
            
            self.f_x_reconstruction_expectation,\
            self.f_x_reconstruction_expectation_log  = self.get_reconstruction(self.f_x_hidden_prob, 
                                                                                 self.sources, 
                                                                                 self.source_lengths)
                
            self.f_x_output_predicted = tf.argmax(self.f_x_prob,axis=-1)    
        
            
    def get_reconstruction(self, input_prob_softmax, sentences ,lengths):
            
        input_scan = (input_prob_softmax, sentences)
        def get_output_reconstruction(value):
            prob = value[0]
            sent = value[1]
            
            prob = tf.gather(prob, sent, axis=-1)
            prob = prob * tf.eye(tf.shape(prob)[0], tf.shape(prob)[1], dtype= self.tf_float_dtype)
            prob = tf.reduce_sum(prob, axis=-1)
            
            return prob, 0
        
        output_prob, _ = tf.map_fn(get_output_reconstruction,input_scan, dtype=(self.tf_float_dtype,
                                                                       tf.int32))
        
        
        output_mask = tf.sequence_mask(lengths=lengths, maxlen=tf.shape(output_prob)[1],
                                       dtype=self.tf_float_dtype, name='mask')
        
        reconstruction_expectation = output_prob * output_mask
        reconstruction_expectation_log = tf.log(output_prob) * output_mask
        
        return reconstruction_expectation, reconstruction_expectation_log
...
# =============================================================================
    ...
    def get_alignment_expectation(self,
        ...
        return alignment_expectation, emission
...
# =============================================================================
    def print_log(self, update_info, start_time):
        ...
        cost_agreement_non_null = update_info[8]
        y_null_cost = update_info[9]
        x_null_cost = update_info[10]
        ...
        if self.global_step.eval() % self.FLAGS.display_freq == 0:
            ...
            self.log.info('Epoch %d , Step %d , Cost agreement: (Non-null cost: %.5f, T-null: %.5f, S-null: %.5f) in %ds at %s', 
                          self.global_epoch_step.eval(),
                          self.global_step.eval(),
                          cost_agreement_non_null,y_null_cost,x_null_cost,
                          time_output,
                          tools.get_time_now())
    ...
    outputs[5], outputs[6], outputs[7], outputs[8], \
    ...
    def decode_reconstruction(self,
        ...
        output_feed = [self.e_y_output_predicted,
                       self.e_y_reconstruction_expectation,
                       self.f_x_output_predicted,
                       self.f_x_reconstruction_expectation
                       ]
        #----------------------------------------------------------------------
        outputs = self.sess.run(output_feed, input_feed)
        
        return np.array(outputs[0]), np.array(outputs[1]),np.array(outputs[2]), np.array(outputs[3])
    ...
    def get_cost_evaluation(self,
        ...
        output_feed = [self.f_y_cost_alignment_expectation,
                       self.e_y_cost_reconstruction_expectation,
                       self.y_KL_divergence_eval,
                       self.e_x_cost_alignment_expectation,
                       self.f_x_cost_reconstruction_expectation,
                       self.x_KL_divergence_eval,
                       self.cost_agreement_non_null,
                       self.y_null_cost,
                       self.x_null_cost]
...
#            print("Variable: ", k, "Shape: ", v.shape)
        ...
        if self.FLAGS.valid_freq!= 0 and self.global_step.eval() % self.FLAGS.valid_freq == 0:
            ...
            for valid_seq in self.valid_set:
                ...
                for idx in range(1):
                    ...
                    for index_word, (predited, correct) in enumerate(zip(eval_info[2][idx], source)):
                ...
                x_reconstruction_set.append(np.prod(eval_info[3]))
    ...
    def evaluate_cost(self):
        ...
        if self.FLAGS.valid_freq!= 0 and self.global_step.eval() % self.FLAGS.valid_freq == 0:
            ...
            cost_agreement_non_null_batch = []
            y_null_cost_batch = []
            x_null_cost_batch = []
                        
            for valid_seq in self.valid_set:
                batch = self.prepare_batch(*valid_seq)
                eval_info = self.get_cost_evaluation(*batch)
                ...
                cost_agreement_non_null_batch.append(eval_info[6])
                y_null_cost_batch.append(eval_info[7])
                x_null_cost_batch.append(eval_info[8])
            ...
            cost_agreement_non_null = np.mean(cost_agreement_non_null_batch)
            y_null_cost = np.mean(y_null_cost_batch)
            x_null_cost = np.mean(x_null_cost_batch)
            ...
            self.log.info('VALIDATION Agreement : Epoch %d , Step %d , Non-null : %.5f, T-null: %.5f, S-null: %5f',
                          self.global_epoch_step.eval(),
                          self.global_step.eval() , 
                          cost_agreement_non_null, 
                          y_null_cost,x_null_cost)
