...
from model.model_variational_based.model_variational_bpe_hmm import Model
...
class Model(Model):
    ...
    def eval(self,
        ...
        for n in range(self.FLAGS.latent_variable_number):
            ...
            transition_finals.append(np.array(outputs[1][0]))
        
        emission_final = np.mean(emission_finals, axis=0)
        
        initial_transitions = []
        transitions = []
        for transition_final in transition_finals:
            initial_transition, transition = self.get_transition_evaluation_nn(transition_final,
                                                                            target_length)
            initial_transitions.append(initial_transition)
    ...
    def viterbi(self, observations, N, pis, Ts, E_s):
        deltas = []
        for E_, pi, T in zip(E_s, pis, Ts):
